...
|---|---|---|
| Dolby Vision P5 / P20 / P10.0 (IPT-PQ-C2 base layer, `bl_signal_compatibility_id == 0`) | **Implemented: GPU reshaping from the per-frame RPU** (issue #207). Neither path can display the base layer as-is: VideoToolbox decodes the samples faithfully, the display reads IPT as BT.2020 YCbCr, and the picture is pink/green — Apple's own `AVAssetImageGenerator` renders `DVprofile20.mp4` exactly as tinted on macOS, so handing the stream to Apple is not a way out. | `VideoDecoder.deliver` routes every `AVCOL_SPC_IPT_C2` frame through `DolbyVisionReshaper`: a Metal compute kernel reads `AV_FRAME_DATA_DOVI_METADATA` (polynomial + MMR reshaping curves, `ycc_to_rgb` matrix/offset, `rgb_to_lms` crosstalk), applies the libplacebo algorithm (reshape → nonlinear matrix → PQ EOTF → inverse crosstalk × BT.2020 HPE LMS→RGB → PQ OETF) and writes a pooled `x420` buffer tagged BT.2020 / PQ / BT.2020 — ordinary HDR10 from there on. Works on the VideoToolbox surface in place (Metal texture cache) and on `PixelBufferFactory` buffers alike. A frame without an RPU reuses the last one; no Metal device or a GPU failure delivers the untouched frame and logs `dv-reshape skipped|failed` once — degraded picture, never a dead pipeline. Not covered: the enhancement layer of dual-layer profiles (4/7), and HDR10 mastering metadata derived from DV L6 (only static metadata the source already carries is copied). P8.1 needs none of this — its base layer is HDR10. |
